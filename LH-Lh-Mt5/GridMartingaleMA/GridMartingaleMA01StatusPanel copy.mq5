//+------------------------------------------------------------------+
//| GridMartingaleMA01StatusPanel.mq5                                |
//| Reads Python-exported state, draws panel and TP lines in MT5.    |
//+------------------------------------------------------------------+
#property strict
#property version   "1.00"
#property description "GridMartingaleMA01 MT5 panel / TP lines / close buttons"

#include <Trade/Trade.mqh>

input ulong  InpMagic            = 2025890;
input string InpStatusExportFolder = "GridMartingaleMA01"; // 与 Python bridge 的 strategy_name 一致；利滚利选 CompoundMartin
...
string StatusRelativePath(const bool account_scoped=true)
  {
   ...
   if(StringLen(folder)<=0)
      folder="GridMartingaleMA01";
   string base="NeuralQuant\\"+folder+"\\";
   if(account_scoped)
      base=base+AccountScopeName()+"\\";
   return base + SafeSymbolName(StatusSymbol()) + "_" + IntegerToString((int)InpMagic) + ".txt";
  }
...
void PutDefaultStatus()
  {
   ResetPairs();
   PutPair("strategy","GridMartingaleMA01");
...
void UpdateRichPanel()
  {
   ...
   const string aiRegime=GetValue("ai_regime","unknown");
   const string aiFamily=GetValue("ai_trend_family","none");
   ...
   const string aiDirTxt=(aiDir=="BUY" ? "看多" : (aiDir=="SELL" ? "看空" : "中性"));
   ...
   const string shockTxt=ShockLabel(shock);
   ...
   PanelTextScroll(g_prefix+"_sec_ai",14,y,g_panel_scroll,"【Ai智能趋势/短线/未来预警】",clrYellow,11,true); y+=18;
   PanelTextScroll(g_prefix+"_ai_big",18,y,g_panel_scroll,"趋势偏向: "+aiDirTxt+" / 短线: "+currentStateTxt,aiDirClr,11,true); y+=17;
   PanelTextScroll(g_prefix+"_ai1",18,y,g_panel_scroll,"趋势状态",clrSlateGray,9,false);
   PanelTextScroll(g_prefix+"_ai1v",150,y,g_panel_scroll,aiRegimeTxt,aiDirClr,9,true); y+=14;
   PanelTextScroll(g_prefix+"_ai2",18,y,g_panel_scroll,"趋势偏向",clrSlateGray,9,false);
   PanelTextScroll(g_prefix+"_ai2v",150,y,g_panel_scroll,aiDirTxt,aiDirClr,9,true); y+=14;
   PanelTextScroll(g_prefix+"_ai3",18,y,g_panel_scroll,"当前短线",clrSlateGray,9,false);
   PanelTextScroll(g_prefix+"_ai3v",150,y,g_panel_scroll,currentStateTxt+" / "+currentDirTxt,(currentDirection=="BUY"?clrLime:(currentDirection=="SELL"?clrTomato:clrSilver)),9,true); y+=14;
   PanelTextScroll(g_prefix+"_ai3b",18,y,g_panel_scroll,"趋势级别",clrSlateGray,9,false);
   PanelTextScroll(g_prefix+"_ai3bv",150,y,g_panel_scroll,aiFamilyTxt,aiDirClr,9,true); y+=14;
   PanelTextScroll(g_prefix+"_ai3c",18,y,g_panel_scroll,"当前异动",clrSlateGray,9,false);
   PanelTextScroll(g_prefix+"_ai3cv",150,y,g_panel_scroll,impulseTxt,impulseClr,9,true); y+=14;
   string impulseMeta="-";
   if(impulseState!="none")
      impulseMeta=StringFormat("%.0f%% / %d / %.2fATR",impulseConf*100.0,impulseAge,impulseExt);
   PanelTextScroll(g_prefix+"_ai3d",18,y,g_panel_scroll,"异动强度",clrSlateGray,9,false);
   PanelTextScroll(g_prefix+"_ai3dv",150,y,g_panel_scroll,impulseMeta,impulseClr,9,true); y+=14;
   ...
   string learnText=StringFormat("%d笔 胜%.0f%% 误%.0f%%",learnSamples,learnSuccess*100.0,learnFalse*100.0);
   PanelTextScroll(g_prefix+"_ai7",18,y,g_panel_scroll,"AI自学统计",clrSlateGray,9,false);
   PanelTextScroll(g_prefix+"_ai7v",150,y,g_panel_scroll,learnText,(learnReady?clrLime:clrSilver),9,true); y+=14;
   string learnState=(learnReady ? "已启用" : "积累中");
   if(learnPending>0)
      learnState=learnState+" / 待验"+IntegerToString(learnPending);
   if(learnLast!="none")
      learnState=learnState+" / 上次"+LearningOutcomeLabel(learnLast);
   PanelTextScroll(g_prefix+"_ai8",18,y,g_panel_scroll,"AI自学状态",clrSlateGray,9,false);
   PanelTextScroll(g_prefix+"_ai8v",150,y,g_panel_scroll,learnState,(learnReady?clrLime:clrSilver),9,true); y+=20;
