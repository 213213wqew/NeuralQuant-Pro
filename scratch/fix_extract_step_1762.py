                    ...
                    if isinstance(args, dict):
                        ...
                        if isinstance(chunks, str):
                            try:
                                ...
                                print("Error decoding chunks string:", ex)
                                # Let's try to extract it by cleaning up control characters if any
                                import re
                                cleaned = re.sub(r'[\x00-\x1f\x7f-\x9f]', '', chunks)
                                try:
                                    chunks = json.loads(cleaned)
                                except Exception as ex2:
                                    print("Cleaned chunks decode failed too:", ex2)
                                    continue
